...
int cutTheTree(vector<int> data, vector<vector<int>> edges) {
    ...
    // Calculate the total sum of all values in the tree
    totalSum = accumulate(data.begin(), data.end(), 0);
    ...
    function<int(int)> dfs = [&](int node) {
        ...
        if (node != 1) {
            ...
            ans = min(ans, abs(subtreeSum - otherSubtreeSum));
        }
